...
@pd.api.extensions.register_dataframe_accessor("nwis")
class NWISFrame:
    ...
    @properties cannot be set by assignment e.g. STAID = "12301933" or self._obj._metadata.STAID = "12301933"
    ...
    def __init__(self, pandas_obj):
        # self._obj._metadata._validate(pandas_obj)
        self._obj = pandas_obj
        # self.staid = Unknown
        # self.start_date = Unknown
        # self.end_date = Unknown
        # self.param = Unknown
        # self.stat_code = Unknown
        # self.service = Unknown
        # self.access = Unknown
        # self.url = Unknown
        # self.gap_tol = Unknown
        # self.gap_fill = Unknown
        # self.resolve_masking = Unknown
        # self._approval = Unknown
    ...
    @property
    def staid(self):
        return str(self._metadata.get("_staid"))

    @property
    def start_date(self):
        return str(self._metadata.get("_start_date"))

    @property
    def end_date(self):
        return str(self._metadata.get("_end_date"))

    @property
    def param(self):
        return self._metadata.get("_param")

    @property
    def stat_code(self):
        return self._metadata.get("_stat_code")

    @property
    def service(self):
        return self._metadata.get("_service")

    @property
    def url(self):
        return self._metadata.get("_url")

    @property
    def site_name(self):
        return self._metadata.get("_site_name")

    @property
    def coords(self):
        return self._metadata.get("_coords")

    @property
    def var_description(self):
        return self._metadata.get("_var_description")

    @property
    def gap_tolerance(self):
        return self._metadata.get("_gap_tolerance")

    @property
    def gaps(self):
        return self._obj._metadata.gap_index(self._obj._metadata.gap_tolerance)

    @property
    def approval(self):
        return self._obj._metadata.check_approval()

    @property
    def qualifier(self):
        return self._obj._metadata.check_quals()
    ...
    def check_gaps(
        ...
        gap_tol : str | None, optional
            ...
            If no gap tolerance is specified, fall back on self._obj._metadata.gap_tolerance property
            If no self._obj._metadata.gap_tolerance property, return NWIS.Unknown sentinel object.
        ...
        bool | None
            ...
            If neither gap_tol or self._obj._metadata.gap_tolerance property are specified,
            returns sentinel object: Unknown
    ...
    def gap_index(
        ...
        return idx.difference(self._obj.index)
    ...
    def fill_gaps(
        ...
        """
        gap_tol = self._obj._resolve_gaptolerance(gap_tol)
        try:
            self = self._obj.reindex(pd.date_range(self.start_date, self.end_date, freq=gap_tol)).rename_axis(["dateTime"]).fillna(float("NaN"))
            # self = self._obj._metadata.asfreq(freq=gap_tol)
            self._obj._metadata._metadict["_gap_tolerance"] = gap_tol
            # self["qualifiers"][self["qualifiers"].isnull()] = Unknown
    ...
    def check_approval(self) -> str:
        ...
        self._obj["_approval"] = approval_level
    ...
    def resolve_masks(self) -> None:
        ...
        self._obj["value"] = self._obj["value"].where(self._obj["value"] != -999999.0, np.NaN)
        return None
    ...
    def _resolve_gaptolerance(
        ...
        """
        If no gap_tol, fall back on self._obj._metadata.gap_tolerance property.

        If gap_tol is None and self._obj._metadata.gap_tolerance is None, return "Unknown" sentinel object.
        """
        if gap_tol is None:
...
def create_metadict(
    ...
    **kwargs,
    ...
    if rdata:
        metadict.update(
            {
                "_site_name": rdata["value"]["timeSeries"][0]["sourceInfo"]["siteName"] or Unknown,
                "_coords": (
                    rdata["value"]["timeSeries"][0]["sourceInfo"]["geoLocation"]["geogLocation"]["latitude"] or Unknown,
                    rdata["value"]["timeSeries"][0]["sourceInfo"]["geoLocation"]["geogLocation"]["longitude"] or Unknown,
                ),
                "_var_description": rdata["value"]["timeSeries"][0]["variable"]["variableDescription"] or Unknown,
            }
        )
    return metadict
...
def get_nwis(
    ...
    dataframe.nwis._metadata = create_metadict(
        dataframe=dataframe,
        STAID=staid,
        start_date=start_date,
        end_date=end_date,
    ...
    if gap_fill and bool(gap_tol):
        dataframe = dataframe.nwis.fill_gaps(gap_tol)
    if resolve_masking:
        dataframe.nwis.resolve_masks()
    dataframe.nwis.check_quals()
    dataframe.nwis.check_approval()
    dataframe.nwis.check_gaps(gap_tol=gap_tol)
    return dataframe
